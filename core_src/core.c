...
void player_open_file(char* filename, int track) {
    handle_error(gme_open_file(filename, &emu, 44100));
    handle_error(gme_start_track(emu, track));
}

void player_start_track(int track) {
    handle_error(gme_start_track(emu, track));
}
void player_seek_to(int seconds) {
    handle_error(gme_seek(emu, seconds));
}
int player_track_ended() {
    return gme_track_ended(emu);
}
short* player_generate_sound_data() {
    handle_error(gme_play(emu, 8192 * 2, audio_buffer));
    return audio_buffer;
}

char* player_song_info(int track) {
