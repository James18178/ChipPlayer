...
class ChipPlayer {
    /**
     * chipplayer constructor function
     *
    ...
    constructor(options) {
        ...
        this.container.classList.add('chipplayer');
        if (this.options.lrcType && !this.options.fixed) {
            this.container.classList.add('chipplayer-withlrc');
        }
        if (this.options.audio.length > 1) {
            this.container.classList.add('chipplayer-withlist');
        }
        if (utils.isMobile) {
            this.container.classList.add('chipplayer-mobile');
        }
        this.arrow = this.container.offsetWidth <= 300;
        if (this.arrow) {
            this.container.classList.add('chipplayer-arrow');
        }

        // save lrc
        if (this.options.lrcType === 2 || this.options.lrcType === true) {
            const lrcEle = this.container.getElementsByClassName('chipplayer-lrc-content');
        ...
        if (this.options.fixed) {
            this.container.classList.add('chipplayer-fixed');
        ...
        if (this.template.info.offsetWidth < 200) {
            this.template.time.classList.add('chipplayer-time-narrow');
        }
        ...
        if (this.options.autoplay) {
            console.warn("can't use autoplay!");
        }

        instances.push(this);
    }
    initAudio() {
        this.playerCore.include(this.options,this.template,this.events);
        this.audio = this.playerCore;
    }
    ...
    setUIPlaying() {
        if (this.paused) {
            this.paused = false;
            this.template.button.classList.remove('chipplayer-play');
            this.template.button.classList.add('chipplayer-pause');
    ...
    setUIPaused() {
        if (!this.paused) {
            ...
            this.template.button.classList.remove('chipplayer-pause');
            this.template.button.classList.add('chipplayer-play');
            this.template.button.innerHTML = '';
            setTimeout(() => {
        ...
        this.container.classList.remove('chipplayer-loading');
     ...
     */
    toggle() {
        if (this.template.button.classList.contains('chipplayer-play')) {
            this.play();
        } else if (this.template.button.classList.contains('chipplayer-pause')) {
            this.pause();
        }
    }
    ...
    setMode(mode = 'normal') {
        this.mode = mode;
        if (mode === 'mini') {
            this.container.classList.add('chipplayer-narrow');
        } else if (mode === 'normal') {
            this.container.classList.remove('chipplayer-narrow');
        }
    }
