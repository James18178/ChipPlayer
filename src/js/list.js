...
class List {
    ...
    bindEvents() {
        this.player.template.list.addEventListener('click', (e) => {
            ...
            const audioIndex = parseInt(target.getElementsByClassName('chipplayer-list-index')[0].innerHTML) - 1;
    ...
    add(audios) {
        ...
        if (wasSingle && this.audios.length > 1) {
            this.player.container.classList.add('chipplayer-withlist');
        }

        this.player.randomOrder = utils.randomOrder(this.audios.length);
        this.player.template.listCurs = this.player.container.querySelectorAll('.chipplayer-list-cur');
    ...
    remove(index) {
        ...
        if (this.audios[index]) {
            if (this.audios.length > 1) {
                const list = this.player.container.querySelectorAll('.chipplayer-list li');
                list[index].remove();
                ...
                for (let i = index; i < list.length; i++) {
                    list[i].getElementsByClassName('chipplayer-list-index')[0].textContent = i;
                }
                if (this.audios.length === 1) {
                    this.player.container.classList.remove('chipplayer-withlist');
                }
    ...
    switch(index) {
        ...
        if (typeof index !== 'undefined' && this.audios[index]) {
            ...
            const light = this.player.container.getElementsByClassName('chipplayer-list-light')[0];
            if (light) {
                light.classList.remove('chipplayer-list-light');
            }
            this.player.container.querySelectorAll('.chipplayer-list li')[this.index].classList.add('chipplayer-list-light');
