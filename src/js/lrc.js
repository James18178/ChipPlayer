...
class Lrc {
    ...
    show() {
        ...
        this.player.template.lrcWrap.classList.remove('chipplayer-lrc-hide');
    }

    hide() {
        this.player.events.trigger('lrchide');
        this.player.template.lrcWrap.classList.add('chipplayer-lrc-hide');
    }

    toggle() {
        if (this.player.template.lrcWrap.classList.contains('chipplayer-lrc-hide')) {
            this.show();
        } else {
            this.hide();
        }
        ...
        if (this.index > this.current.length - 1 || currentTime < this.current[this.index][0] || (!this.current[this.index + 1] || currentTime >= this.current[this.index + 1][0])) {
            for (let i = 0; i < this.current.length; i++) {
                if (currentTime >= this.current[i][0] && (!this.current[i + 1] || currentTime < this.current[i + 1][0])) {
                    ...
                    this.container.getElementsByClassName('chipplayer-lrc-current')[0].classList.remove('chipplayer-lrc-current');
                    this.container.getElementsByTagName('p')[i].classList.add('chipplayer-lrc-current');
                }
            }
        }
    }
